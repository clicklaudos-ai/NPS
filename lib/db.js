...
function mapLink(row) {
  return {
    ...
    statusEnvio: row.status_envio || 'ok',
    erroEnvio: row.erro_envio || null,
...
/* Escrita — Links                                                      */
/* ------------------------------------------------------------------ */
export async function insertLink({ id, cliente, atendente, tipo, url, email, canal, statusEnvio, erroEnvio }) {
  const payload = { cliente, atendente_nome: atendente, tipo, url };
  if (id) payload.id = id;
  if (email) payload.email = email;
  if (canal) payload.canal = canal;
  if (statusEnvio) payload.status_envio = statusEnvio;
  if (erroEnvio) payload.erro_envio = erroEnvio;
